...
extern u8 n,v;
extern u16 I_Positive,I_DPositive,i_in,i_de,ii,vl;
...
//		if(T[0]>=sys_data[26]||T[1]>=sys_data[26]||T[2]>=sys_data[26]||T[3]>=sys_data[26])
		if(T[0]>=sys_data[26]||T[1]>=sys_data[26])
		{
			if(temalarm_on>=6)
			{
				temalarm_on=6;
...
//		}
	...
	if( (sys_data[0]>=sys_data[32]&&(I_Positive<ii)&&(sys_data[0]<=700) ) || sys_data[37]==1)
	{	
		if (n>6)
		{
			sys_data[2]=1000;
	...
	if (	(sys_data[0]<=vl)&&(sys_data[0]>=100)	)
	{	
		if (v>60)
		{
			sys_data[2]=0;		
