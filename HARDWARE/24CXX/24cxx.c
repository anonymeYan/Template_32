...
{
    u8 temp;
    temp=FM24Cl04_ReadOneByte(511);//避免每次开机都写AT24CXX
...
u8 FM24Cl04_ReadOneByte(u16 ReadAddr)
{
    ...
    if(EE_TYPE>AT24C16)
    {
        MYIIC_IIC1.MYIIC_Send_Byte(0XA8);	   //发送写命令
        MYIIC_IIC1.MYIIC_Wait_Ack();
        MYIIC_IIC1.MYIIC_Send_Byte(ReadAddr>>8);//发送高地址
        MYIIC_IIC1.MYIIC_Wait_Ack();
    } else MYIIC_IIC1.MYIIC_Send_Byte(0XA8+((ReadAddr/256)<<1));   //发送器件地址0xA8,写数据

    MYIIC_IIC1.MYIIC_Wait_Ack();
    MYIIC_IIC1.MYIIC_Send_Byte(ReadAddr%256);   //发送低地址
    MYIIC_IIC1.MYIIC_Wait_Ack();
    MYIIC_IIC1.MYIIC_Start();
    MYIIC_IIC1.MYIIC_Send_Byte(0XA9);           //进入接收模式
    MYIIC_IIC1.MYIIC_Wait_Ack();
...
void FM24Cl04_WriteOneByte(u16 WriteAddr,u8 DataToWrite)
{
    ...
    if(EE_TYPE>AT24C16)
    {
        MYIIC_IIC1.MYIIC_Send_Byte(0XA8);	    //发送写命令
        MYIIC_IIC1.MYIIC_Wait_Ack();
        MYIIC_IIC1.MYIIC_Send_Byte(WriteAddr>>8);//发送高地址
    } else
    {
        MYIIC_IIC1.MYIIC_Send_Byte(0xA8+((WriteAddr/256)<<1));   //发送器件地址0xA8,写数据
    }
    MYIIC_IIC1.MYIIC_Wait_Ack();
    MYIIC_IIC1.MYIIC_Send_Byte(WriteAddr%256);   //发送低地址
    MYIIC_IIC1.MYIIC_Wait_Ack();
    MYIIC_IIC1.MYIIC_Send_Byte(DataToWrite);     //发送字节
    MYIIC_IIC1.MYIIC_Wait_Ack();
    MYIIC_IIC1.MYIIC_Stop();;//产生一个停止条件
    delay_ms(10);
}
